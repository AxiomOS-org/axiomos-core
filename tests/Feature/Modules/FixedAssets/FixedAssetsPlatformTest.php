...
namespace Tests\Feature\Modules\FixedAssets;
use App\Core\Http\HttpKernelFactory;
use Illuminate\Http\Request;
use Symfony\Component\HttpFoundation\Response;
use Tests\Support\Stability\KernelTestHarness;
final class FixedAssetsPlatformTest extends KernelTestHarness {
    public function test_module_routes_do_not_return_500(): void {
        ...
        $failures = [];
        foreach ([
            '/api/assets/fixed-asset?company_id=' . $companyId,
            '/api/assets/depreciation-run?company_id=' . $companyId,
            '/assets',
            '/assets/dashboard',
        ] as $path) {
