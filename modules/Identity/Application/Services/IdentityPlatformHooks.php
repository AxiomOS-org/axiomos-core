...
use Modules\Identity\Application\Events\DomainEventRecorder;
use Modules\Identity\Domain\Events\RecordLifecycleEvent;

/**
 * Full enterprise platform lifecycle integration for Identity entities.
 */
final class IdentityPlatformHooks
{
    ...
    public function onCreated(PlatformEntityInterface $entity): void
    {
        ...
        $this->aiContext->upsert($entity, 'default', ['entity_type' => $entity->getEntityType(), 'summary' => 'Identity platform record']);
        ...
        $this->notes->create($entity, 'Identity platform record provisioned.', 'Lifecycle');
        $this->comments->create($entity, 'System lifecycle comment for audit trail.');
        $this->attachments->create($entity, [
            'disk' => 'local',
            'path' => 'identity/' . $entity->getEntityId() . '/manifest.json',
            'filename' => 'manifest.json',
            'mime_type' => 'application/json',
            'size_bytes' => 2,
            'metadata' => ['source' => 'identity-platform'],
        ]);
        $this->tags->attach($entity, 'identity-platform', scope: 'identity');
