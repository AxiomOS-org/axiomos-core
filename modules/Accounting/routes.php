...
return static function (Router $router, ContainerInterface $container): void {
    ...
    $router->post('/api/accounting/fiscal-years', static fn(Request $request) => $api->fiscalYears($request));
    $router->get('/api/accounting/periods', static fn(Request $request) => $api->periods($request));
    $router->post('/api/accounting/periods', static fn(Request $request) => $api->periods($request));
    $router->patch('/api/accounting/periods', static fn(Request $request) => $api->periods($request));
    ...
    $router->get('/api/accounting/posting-logs', static fn(Request $request) => $api->postingLogs($request));
    ...
    $router->post('/api/accounting/dimensions/cost-centers', static fn(Request $request) => $api->costCenters($request));
    ...
    $router->post('/api/accounting/exchange-rates', static fn(Request $request) => $api->exchangeRates($request));
