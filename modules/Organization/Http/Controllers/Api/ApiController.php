...
use Modules\Organization\Http\Support\FormRequestValidator;
...
abstract class ApiController
{
    ...
    protected function collection(Collection $items): JsonResponse
    {
        return new JsonResponse([
            'data' => $items->map(static fn ($item): array => OrganizationResource::base($item))->values()->all(),
        ]);
    }
    ...
    protected function paginated(\Illuminate\Contracts\Pagination\LengthAwarePaginator $paginator): JsonResponse
    {
        return new JsonResponse([
            'data' => collect($paginator->items())
                ->map(static fn ($item): array => OrganizationResource::base($item))
                ->values()
                ->all(),
            'meta' => [
    ...
    {
        return new JsonResponse(['data' => OrganizationResource::base($model)], $status);
    }
}
