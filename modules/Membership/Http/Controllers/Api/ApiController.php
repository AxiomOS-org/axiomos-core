...
use Modules\Membership\Http\Resources\MembershipResource;
use Modules\Membership\Http\Support\FormRequestValidator;
...
abstract class ApiController
{
    ...
    protected function collection(Collection $items): JsonResponse
    {
        return new JsonResponse([
            'data' => $items->map(static fn ($item): array => MembershipResource::base($item))->values()->all(),
        ]);
    }
    ...
    protected function paginated(\Illuminate\Contracts\Pagination\LengthAwarePaginator $paginator): JsonResponse
    {
        return new JsonResponse([
            'data' => collect($paginator->items())
                ->map(static fn ($item): array => MembershipResource::base($item))
                ->values()
                ->all(),
            'meta' => [
    ...
    {
        return new JsonResponse(['data' => MembershipResource::base($model)], $status);
    }
}
