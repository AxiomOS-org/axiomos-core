...
use Modules\Users\Application\Events\DomainEventRecorder;
use Modules\Users\Domain\Events\RecordLifecycleEvent;

final class UsersPlatformHooks
{
    ...
    public function onCreated(PlatformEntityInterface $entity): void
    {
        ...
        $this->aiContext->upsert($entity, 'default', ['entity_type' => $entity->getEntityType(), 'summary' => 'User record']);
        ...
        $this->notes->create($entity, 'User record provisioned.', 'Lifecycle');
        $this->comments->create($entity, 'System lifecycle comment for audit trail.');
        $this->attachments->create($entity, ['disk' => 'local', 'path' => 'users/' . $entity->getEntityId() . '/manifest.json', 'filename' => 'manifest.json', 'mime_type' => 'application/json', 'size_bytes' => 2]);
        $this->tags->attach($entity, 'users', scope: 'identity');
