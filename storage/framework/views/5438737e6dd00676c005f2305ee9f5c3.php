...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted mb-0">Branch departments — powered by REST API</p>
    </div>
    ...
    <div class="card-body">
        <form class="row g-3 align-items-end" id="filters-form">
            ...
            <div class="col-md-3">
                ...
                <select class="form-select" name="branch_id" id="filter-branch"></select>
            </div>
            <div class="col-md-2">
    ...
    <div class="modal-dialog modal-lg">
        <form class="modal-content" id="entity-form">
            <div class="modal-header">
                <h5 class="modal-title" id="modal-title">Department</h5>
            ...
            <div class="modal-body row g-3">
                ...
                <div class="col-12">
                    <label class="form-label">Branch *</label>
                    <select class="form-select" name="branch_id" id="form-branch" required></select>
                </div>
                <div class="col-md-4">
...
<script>
(() => {
    const apiBase = '/api/departments';
    const branchApi = '/api/branches';
    let branchMap = {};
    let state = { page: 1, search: '', status: '', branch_id: '', sort: 'name', direction: 'asc' };
    ...
    async function loadBranches() {
        const res = await fetch(`${branchApi}?page=1&per_page=100&sort=name`);
        const json = await res.json();
        branchMap = Object.fromEntries((json.data || []).map(b => [b.id, b.name]));
        const opts = '<option value="">All branches</option>' + (json.data || []).map(b => `<option value="${b.id}">${b.name}</option>`).join('');
        document.querySelector('#filter-branch').innerHTML = opts;
        document.querySelector('#form-branch').innerHTML = (json.data || []).map(b => `<option value="${b.id}">${b.name}</option>`).join('');
    }
    ...
    function buildQuery() {
        ...
        if (state.branch_id) p.set('branch_id', state.branch_id);
        return p.toString();
    }
    ...
    async function load() {
        ...
        tableBody.innerHTML = (json.data || []).map(item => `
            <tr>
                ...
                <td class="small text-muted">${branchMap[item.branch_id] || ''}</td>
    ...
    document.querySelector('#filters-form').addEventListener('submit', e => {
        ...
        state = { ...state, search: fd.get('search') || '', status: fd.get('status') || '', branch_id: fd.get('branch_id') || '', sort: fd.get('sort') || 'name', page: 1 };
        load();
    });
    ...
    tableBody.addEventListener('click', e => {
        ...
        if (editBtn) {
            ...
            document.querySelector('#modal-title').textContent = 'Edit Department';
            formErrors.classList.add('d-none');
            modal.show();
        }
        const delBtn = e.target.closest('[data-delete]');
        if (delBtn && confirm('Delete this department?')) {
    ...
    document.querySelector('#btn-create').addEventListener('click', () => {
        ...
        document.querySelector('#modal-title').textContent = 'New Department';
        formErrors.classList.add('d-none');
    });
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\AxiomOS\axiomos-core\modules\Organization\Resources\views/departments/index.blade.php ENDPATH**/ ?>