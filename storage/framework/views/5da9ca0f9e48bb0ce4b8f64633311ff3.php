...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted mb-0">Manage <?php echo e(strtolower($entityLabel)); ?> through Security APIs.</p>
    </div>
...
(() => {
    ...
    const readOnly = <?php echo json_encode($readOnly ?? false, 15, 512) ?>;
    ...
    const modal = readOnly ? null : bootstrap.Modal.getOrCreateInstance(document.querySelector('#entity-modal'));
    ...
    function buildForm() {
        ...
        const controls = fields.map(field => {
            if (field === 'is_system') {
                return `<div class="col-md-6"><label class="form-label">Is System</label><select class="form-select" name="is_system"><option value="1">Yes</option><option value="0">No</option></select></div>`;
            }
    ...
    async function load() {
        tableBody.innerHTML = `<tr><td colspan="${columns.length + (readOnly ? 0 : 1)}" class="text-center py-4 text-muted">Loading...</td></tr>`;
        ...
        tableBody.innerHTML = rows || `<tr><td colspan="${columns.length + (readOnly ? 0 : 1)}" class="text-center py-4 text-muted">No records found.</td></tr>`;
    ...
    if (!readOnly) {
        ...
        tableBody.addEventListener('click', event => {
            ...
            if (edit) {
                ...
                for (const field of fields) {
                    ...
                    if (!input) continue;
                    input.value = field === 'is_system' ? (item[field] ? '1' : '0') : (item[field] ?? '');
                }
        ...
        form.addEventListener('submit', async event => {
            ...
            for (const field of fields) {
                ...
                if (value === '') continue;
                payload[field] = field === 'is_system' ? value === '1' : value;
            }
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\AxiomOS\axiomos-core\modules\Authorization\Resources\views/crud/index.blade.php ENDPATH**/ ?>